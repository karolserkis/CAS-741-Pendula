...
\maketitle
\wss{The name of your program ``\progname{}'' is rather long and awkward.  Do
  you think you could come up with a shorter name, maybe MPS?}
...
\wss{All figures should be referred to by your text at some point.}
...
their units. The choice of symbols was made to be consistent with calculus, ordinary
differentials (ODE), the Lagrangian, kinematics etc. The standard mathematical spaces
...
\bottomrule
\end{longtable*}
\wss{Poincaré in \LaTeX{} is written as Poincar\'{e}}
\newpage
...
This documents is an SRS for the \progname \wss{Your prog name isn't exactly
  catchy; it seems more like a description than a program name.} program. The
...
[8]. \wss{Use BibTex and citation command.  Follow the example in the blank
  project template}
...
and validation plan. The verification and validation plan will show the steps
inthe \wss{proof read, spell check} software documentation/implementation.
...
scientific
computing software proposed by Dr. Spencer Smith. \wss{You have a citation
  below.  Having this point too is redundant.}
...
\item This document follows the template outlined in Smith and Lai (2005)
  [8]. \wss{You can use BibTeX to make it easy to manage citations and
    references. Hard coding in reference will be a maintenance nightmare.}
\end{itemize}
\newpage
\wss{Do you really need to hardcode in a newpage here?  \LaTeX{} usually makes
  good decisions about pagebreaks.}
...
\wss{Can you characterize the inputs better than just inputs?  What values are
  going to be input?  Similarly for the outputs.}
...
multi-platform support.  \wss{Your current documentation has an implied
  constraint.   You keep mentioning plots.  This is not the only way to output
  data.  It would be more abstract to not impose this on the software.  If you
  cannot be more abstract, then you should make the display requirement a system constraint.}
...
statistics over time to the user. \wss{I believe you are assuming no friction,
...
In the model we assume no air resistance with a frictionless pivot, moving in
space with respect to the Lagrangian caluations \wss{spell check} made. [T1][DD1][IM1] \wss{You
  need to explicitly invoke these assumptions when they apply, not just list them.}
...
\theta_2)$$
  ...
  Source & [6]\\
  \hline
  Ref.\ By & [A1][A4] \wss{Use cross-references to generate these, rather than hardcoding.}\\
  \hline
\end{tabu}
\end{minipage}
...
\wss{I would think you would have theoretical models on kinetic energy and
  potential energy, defined in an abstract way, so that they would be useful for
  other problems.}
\wss{These models are for a double pendulum, but the problem is supposed to have
  more than two pendula.}
...
\wss{This isn't enough information for me to understand what a Poincar\'{e} map is.}
...
\item[R\refstepcounter{reqnum}\thereqnum \label{funinput}:] \progname program will 
  take the following inputs: \wss{It would be nice to use symbols to represent
    the inputs.  You also want a way to identify how many massess you have.}
  \begin{enumerate} \item The initial mass of the weights. \wss{This is a
      confusing sentence, since mass and weight are different concepts.  If you
      use a symbol, like $m_i$ then you could probably simplify the presentation.}
                    \item The inital \wss{spell check} length of the rods.
  \end{enumerate}
...
and potential energy of the system as whole has been calculated. \wss{This is
...
labels \& 3D cartesian coordinates. \wss{This isn't actually a nonfunctional
  requirement; it is a functional requirement.}
\end{itemize}
...
\wss{Your NFRs are ambiguous.  You might have been better off with the blanket
  statement in the first paragraph, since the list of qualities brings attention
  to the fact that what you have stated is not measurable.}
...
\wss{Use BibTex.  You can follow the example in the sample document in the blank
project template.  To start with, you can see whether you can compile that
document.}
