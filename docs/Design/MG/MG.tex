...
\item[\refstepcounter{acnum} \actheacnum \label{acFormat}:] The format of the
  initial input data.
...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInput}:] There will always be
  a source of input data external to the software. \wss{You can remove this UC.
    I know it was in the template, but it doesn't make sense to me.  I've
    removed it from the template.}
\item[\refstepcounter{ucnum} \uctheucnum \label{ucGoal}:] The goal statements of \progname
(see SRS; extensions to SRS are still possible, but current goals should not change
\item[\refstepcounter{ucnum} \uctheucnum \label{ucData}:] The data types (real number)
interface device with user \wss{This looks like two unlikely changes in one.
  The physics of the problem is for real numbers, so I'm not sure you need to
  say this.  You already mention input devices in the first UC.}
\end{description}
...
\label{MG_Control}
\begin{description}
\item[Secrets:] Execution flow of \progname{}. \wss{At the end of a sentence you
    want to just use progname without the parentheses.} 
...
\midrule
R1 & M2, M3, M4, M5, M6, M7, M8, M9\\%\mref{mHH}, \mref{mInput}, \mref{mParams}, \mref{mControl}\\
...
\wss{The uses relation is confusing.  The input module should be ``used by'' other
  modules, not using them.  I would think the input module would be lower in the
hierarchy.  I don't know why the trajectory simulation module uses the gui
module?  Maybe this will be clear to me (and you) when you write the MIS.}
